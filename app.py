...
# ==========================================
# 2. デザインカスタマイズ (モバイル・中央揃え)
# ==========================================
st.set_page_config(page_title="O-lys AI評価システム", layout="centered")

st.markdown("""
    <style>
    .main .block-container { text-align: center; padding-top: 2rem; }
    ...
    .stButton>button {
        width: 100% !important; height: 3.5em; font-size: 1.2em !important;
        border-radius: 50px; margin-top: 20px; background-color: #1E90FF; color: white;
    }
    input, textarea, select { font-size: 16px !important; text-align: center; }
    .work-card {
...
with st.expander("📋 障害について入力する"):
    ...
    st.session_state['dis_detail'] = st.text_area("Q2. どんな障害かをおしえてください", value=st.session_state['dis_detail'], placeholder="例：障害者1級。内容はASD、ADHDです。")

st.divider()

# --- ワーク・シミュレーション (縦スクロール形式) ---
...
st.session_state['r_t_val'] = st.text_area("Q. あなたにとって働くことの「お金」以外の意味は何ですか？", value=st.session_state['r_t_val'], key="r_final")
...
st.session_state['w_t_val'] = st.text_area("Q. どんな場面で、どう感じましたか？", value=st.session_state['w_t_val'], key="w_final")
st.markdown('</div>', unsafe_allow_html=True)
...
st.session_state['c_t_val'] = st.text_area("Q. 計算式と答えを書いてください。", value=st.session_state['c_t_val'], key="c_final", placeholder="（書き方の例）時給 × 時間 × 日数 ＝ 合計金額")
st.markdown('</div>', unsafe_allow_html=True)
...
st.session_state['m_t_val'] = st.text_area("Q. セリフを具体的に書いてください。", value=st.session_state['m_t_val'], key="m_final")
st.markdown('</div>', unsafe_allow_html=True)
...
if st.button("🚀 強みを発見する（診断開始）", type="primary"):
    if not st.session_state['name']:
        ...
        with st.spinner("AIが24職種とマッチング中..."):
            # 本来は回答テキストから動的に算出。ここではシミュレーション。
...
if st.session_state['evaluated']:
    ...
    fig_bar.update_traces(texttemplate='<b>%{text}%</b>', textposition='inside', marker_line_color='rgb(8,48,107)', marker_line_width=1.5)
    ...
    st.success(f"**未来へのエール：** 目標である「{st.session_state['life_goal']}」に向けて、あなたの最大の強み「{top_3[0]}」を自信に変えて進んでいきましょう！")
